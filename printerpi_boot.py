...
if __name__ == "__main__":
    ...
    if scheme == None:
        ...
        for cell in cells:
            if cell.ssid == wifi_ssid:
                ...
                log.log("SSID " + wifi_ssid + " was not found. Exiting...")

    if scheme.activate() == False:
        log.log("Could not connect to " + wifi_ssid + ". Exiting...")
        exit(3)
    log.log("Connected!")
    ...
    log.log("Activating printer, watch output of details")
    for i in range(100):
        try:
            r = requests.get(url, timeout=1)
            if r.status_code == requests.codes.ok:
                log.log("Response was okay! Good to go. Exiting...")
                exit(0)
            else:
                #TODO make this more descriptive
                log.log("Response was " + r.status_code
                        + ". Something went wrong")
                exit(4)
        except requests.ConnectionError:
            if i % 5 == 0:
                log.log("Error occured, will try "
                        + str(100 - i) + " more times.")
            sleep(1)
    log.log("No connection after 100 attempts. Call the ghostbusters")
    exit(5)
